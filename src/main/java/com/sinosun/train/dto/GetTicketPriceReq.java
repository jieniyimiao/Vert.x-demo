...
public class GetTicketPriceReq {
    ...
    public void validate() {
        if (StringUtils.isBlank(arriveStation)
                ...
                || departDate == null) {
            throw new IllegalArgumentException("argument arriveStation/departStation/departDate is blank");
        }

        if (new DateTime(departDate).plusDays(1).minusSeconds(1).isBeforeNow()) {
            throw new IllegalArgumentException("argument departDate must more than currment time");
        }
    }
